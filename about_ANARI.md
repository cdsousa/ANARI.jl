...
## Machine-readable API metadata (SDK JSON)

Beyond the textual specification, the [ANARI SDK](https://github.com/KhronosGroup/ANARI-SDK) ships structured JSON files under `code_gen/api/` that describe the API in a machine-readable form. The SDK uses these files (via `merge_anari.py` and various `generate_*.py` scripts) to produce C headers, type utilities, device wrappers, and validation code. The same files are installed with the SDK (e.g. under `share/anari/code_gen/api/` on Linux).

There is no published JSON Schema for this format; it is an internal Khronos convention. The files fall into three categories:

- **Core API** (`anari_core_1_0.json`) — enums (including `ANARIDataType`), C function signatures, opaque handle types, structs, and callback typedefs.
- **Object extensions** (`khr_*.json`, `ext_*.json`, …) — object type/subtype definitions with parameter metadata: allowed types, tags, defaults, enumerated string values, and descriptions.
- **Aggregation** (`anari_core_objects_all_1_0.json`, `anari_core_objects_base_1_0.json`) — dependency lists and base object definitions that tie the extension files together.

Extension files declare dependencies in an `info.dependencies` array; `merge_anari.py` resolves and merges them into a single API tree before code generation.

These JSON files are the most complete **static** description of the ANARI API surface — richer than the generated C headers alone, which cover the core C entry points but not object parameters and subtypes. They complement, rather than replace, runtime introspection: static metadata describes what the specification defines, while `anariGetObjectInfo()` / `anariGetParameterInfo()` report what a particular device actually supports.

A separate XML API registry exists in [ANARI-Docs](https://github.com/KhronosGroup/ANARI-Docs) (`xml/an.xml`), used by the specification build toolchain (header generation, valid-usage extraction). It follows the Khronos/Vulkan registry model but appears less actively maintained than the SDK JSON files for the current API.
