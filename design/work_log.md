...
### Entry 18
- *Summary*: Added runnable `examples/sample.jl` aligned with `design/work_plan.md` (helide triangle scene, inferred `setparam!`, `new_array1d`, render/map/unmap, PNG via `FileIO`/`Images`/`PNGFiles`); moved image dependencies to `examples/Project.toml` + `examples/Manifest.toml` with `[sources]` path to the parent package. Extended `anari_type` / `_prepare_parameter_ref` for `NTuple{3, UInt32}` (`ANARI_UINT32_VEC3`) and for `ANARI_DATA_TYPE` so frames can set `channel.color` to `ANARI_UFIXED8_RGBA_SRGB`. Sample maps the color buffer with `unsafe_wrap`/`reshape`/`transpose` (no copy) and calls `unmap_frame` only after `save` so mapped memory stays valid for PNG encoding.
- *Files*: `examples/sample.jl`, `examples/Project.toml`, `examples/Manifest.toml`, `src/anari_type.jl`, `src/parameters.jl`, `test/anari_type_test.jl`, `design/work_log.md`, `design/ai_chats/10_examples_sample_frame_png.md`
- *Notes*: Run the example with `julia --project=examples examples/sample.jl` from the repo root; requires Julia 1.11+ for `[sources]` in the examples environment. Root `Project.toml` no longer lists `Images`/`FileIO`/`PNGFiles`.

### Entry 19
- *Summary*: Added `ANARI_STRING` parameter support to `setparam!` (both explicit-dtype and inferred paths); added `anari_type(::Type{<:AbstractString})` trait method; preserved the source string during the FFI call with `GC.@preserve`.
- *Files*: `src/parameters.jl`, `src/anari_type.jl`, `test/handles_test.jl`, `test/anari_type_test.jl`, `design/work_log.md`
- *Notes*: String values are converted to `Cstring` via `Ref{Cstring}` so ANARI receives a pointer-to-pointer as required; `AbstractString` dispatch covers `String`, `SubString`, etc.; full test suite passes with `Pkg.test()` (120 tests).
